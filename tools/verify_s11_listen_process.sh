...
if [[ $# -ne 3 ]]; then
    echo "usage: verify_s11_listen_process.sh <graphical-listen-host> <graphical-guest> <content-root>" >&2
    exit 2
fi
...
"$host" --content-root "$content_root" --port "$port" --ticket "$ticket" --max-frames 20000 --s11-observer \
    >"$host_log" 2>&1 &
host_pid=$!
for _ in {1..240}; do
...
"$guest" --content-root "$content_root" --ticket "$ticket" --max-frames 20000 --s11-attacker --s11-listen \
    >"$guest_log" 2>&1 &
guest_pid=$!
wait "$guest_pid"
guest_pid=
