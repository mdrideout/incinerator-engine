#!/usr/bin/env python3
"""Manufacture the direct native 160x90 to 640x360 RF8 corpus."""
...
SEQUENCES = (
    ("overfit", "rf8-overfit-direct-640-0001", "nr4-sequence"),
    ("train", "rf8-train-direct-640-0001", "nr4-corpus-train"),
    ("validation", "rf8-validation-direct-640-0001", "nr4-corpus-validation"),
    ("test", "rf8-test-direct-640-sealed-0001", "camera-cut"),
    ("stress", "rf8-stress-direct-640-near-0001", "nr4-corpus-stress-near"),
    ("stress", "rf8-stress-direct-640-high-0001", "nr4-corpus-stress-high"),
)
...
def main() -> None:
    ...
    if args.resume:
        if not output.is_dir() or load_json(output / "acceptance.json").get("status") != "partial":
            raise ValueError("RF8 resume requires an existing partial root")
    else:
        output = create_absent(output, "RF8 corpus root")
    runs = output / "runs"
    ...
    if not args.resume:
        atomic_json(
            acceptance,
            {
                ...
                "status": "partial",
                "phase": "RF8-B",
                "purpose": "fresh direct native 160x90 to 640x360 whole-sequence corpus",
                "product_review": "approved_2026_08_12",
    ...
    for split, sequence, camera_path in SEQUENCES:
        ...
        if run_root.exists():
            if not args.resume:
                raise FileExistsError(f"RF8 run already exists: {run_root}")
